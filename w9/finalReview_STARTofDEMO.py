...
def swap(listName, posit):
     #swap values
    temp = listName[posit]
    listName[posit] = listName[posit + 1]
    listName[posit + 1] = temp

    #this function could not return OR return TWO values ...
    #return nothing: because lists are stored to memory for the program;when we swap spalces, list is automatically updated

    #python supports multi return values
    #return listName[posit], listName[posit + 1]
...
def search_menu():
    ...
    #while response not in [1,2,3] or
...
#1 - connect to a file and import its data into lists
with open("w9/finalReview_text.csv") as csvfile:
...
while answer == "y":
    ...
    if userChoice == 1:
        ...
        #create an empty list to store found values
        found = []
        ...
        #print results
        if not found: #check to see if list is empty
    ...
    if userChoice == 2:
        ...
        while min < max and search != firstName[mid]:
         ...
         #print results
        if search == firstName[mid]:
            print("\t\nYAY! we found your search")
            print(f"{idCode[mid]:10}\t{lastName[mid]:15}\t{firstName[mid].upper():15}\t{age[mid]:3}\t{allegiance[mid]:35}\t{num[mid]:3}\t{color1[mid]:7}\t{color2[mid]:7}")
        else:
            [print(f"Sorry we could not find {search} :[")]
    if userChoice == 3: 
        print("exit")
